plotVennUI <- function(id) {
  ...
  shiny::tagList(
    shiny::wellPanel(
      tags$h2("Visualize overlap of results across datasets and/or methods"),
      tags$b("Instructions"),
      tags$ol(
        tags$li("Pass one or more diff_tables on each input box"),
        tags$li("Optionally name the 3 ensembles or leave the default name"),
        tags$li("The sign filter does not work for now, so you cannot filter by + - regulation"),
        tags$li("Effect size does not work either")
      )
    ),
    shiny::wellPanel(
    "Inputs"
